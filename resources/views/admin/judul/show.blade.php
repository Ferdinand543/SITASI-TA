...
<style>
    ...
    .info-card {
        ...
        border-left: 4px solid #FFE083; border-radius: 10px;
        padding: 16px 18px; display: flex; align-items: center; gap: 14px;
    }
    .info-icon {
        width: 38px; height: 38px; background: #FFFBEA;
        border-radius: 8px; display: flex; align-items: center;
        justify-content: center; color: #F4B400; font-size: 1rem; flex-shrink: 0;
    }
    .info-label { font-size: 0.7rem; font-weight: 700; letter-spacing: 0.06em; color: #9CA3AF; margin-bottom: 4px; }
    .info-value { font-size: 1rem; font-weight: 600; color: #111; }
    ...
    .info-penting-header { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
    ...
    .judul-card-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
    .usulan-badge { font-size: 0.72rem; font-weight: 700; padding: 3px 10px; border-radius: 20px; }
    .badge-pending   { background: #FEF9C3; color: #92400E; }
    .badge-approved  { background: #DCFCE7; color: #15803D; }
    .badge-rejected  { background: #FEE2E2; color: #B91C1C; }

    .reset-icon { cursor: pointer; color: #9CA3AF; font-size: 0.95rem; transition: color 0.15s; }
    .reset-icon:hover { color: #374151; }

    .judul-main { font-size: 1rem; font-weight: 700; color: #111; margin-bottom: 10px; line-height: 1.45; }
    ...
    .btn-kembali {
        ...
        background: none; color: #374151; border: none;
    ...
    .popup-box { width: 340px; background: #fff; border-radius: 16px; padding: 32px 28px; text-align: center; }
    .popup-icon-wrap {
    ...
    .btn-batal { background: #F3F4F6; color: #374151; margin-left: 8px; }
...
<div class="wrapper-page">
    ...
    <div class="page-subtitle">Tinjau dan berikan keputusan untuk setiap usulan judul mahasiswa.</div>
    ...
    <div class="info-grid">
        ...
        <div class="info-card">
            ...
            <div>
                ...
                <div class="info-value">{{ \Carbon\Carbon::parse($pengajuan->created_at)->format('d M Y') }}</div>
            </div>
        </div>
        <div class="info-card">
            ...
            <div>
                ...
                <div class="info-value status-{{ $st === 'disetujui' ? 'disetujui' : ($st === 'ditolak' ? 'ditolak' : 'menunggu') }}">
                    {{ $st === 'disetujui' ? 'Disetujui' : ($st === 'ditolak' ? 'Ditolak' : 'Menunggu Verifikasi') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="info-penting">
        ...
        <ul class="info-penting-list">
            <li>Admin hanya dapat menyetujui maksimal satu judul dari beberapa usulan yang diajukan oleh mahasiswa.</li>
            <li>Apabila seluruh usulan judul dinilai belum sesuai, admin berhak menolak seluruh pengajuan.</li>
            <li>Keputusan verifikasi yang telah disimpan tidak dapat diubah kembali.</li>
            <li>Pastikan judul yang disetujui relevan dengan topik penelitian dan sesuai standar akademik.</li>
        </ul>
    </div>
    ...
    <form action="{{ route('admin.judul.proses', $pengajuan->id) }}" method="POST" id="formVerifikasi">
        ...
        @php
            if ($sudahDiverifikasi) {
                $isDisetujui = ($j['judul'] === $judulDisetujui);
                $isDitolak   = !$isDisetujui;
                if ($st === 'ditolak') { $isDisetujui = false; $isDitolak = true; }
            } else {
                $isDisetujui = false; $isDitolak = false;
            }
        ...
        <div class="judul-card {{ $cardBorder }}" id="card{{ $no }}">
            <div class="judul-card-top">
                <span class="usulan-badge {{ $badgeClass }}" id="badge{{ $no }}">
...
<div class="footer-sticky">
    <a href="{{ route('admin.judul.index') }}" class="btn-kembali">
        <i class="fa fa-arrow-left"></i> Kembali
    </a>
    @if(!$sudahDiverifikasi)
...
<div class="popup-bg" id="popupBg">
    <div class="popup-box">
        <div class="popup-icon-wrap" id="popupIconWrap"><span id="popupIcon">✓</span></div>
