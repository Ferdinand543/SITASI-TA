...
// ─── KONDISI BARU ────────────────────────────────────────────────────────────
// Hitung berapa dosen yang sudah input nilai
$jumlahSudahNilai = count(array_filter([$nilaiPembimbing1, $nilaiPembimbing2, $nilaiPenguji1]));

// "Penilaian Sedang Berlangsung" → minimal 1 dosen sudah input, tapi belum semua
$sedangBerlangsung = $jumlahSudahNilai > 0 && !$adaPenilaian;

// Cek per-tab apakah dosen di tab itu sudah input atau belum
$tabSudahNilai = match($tabAktif) {
    ...
    default       => $adaPenilaian, // akumulasi: true hanya kalau semua sudah
};
// ─────────────────────────────────────────────────────────────────────────────
...
<style>
    ...
    .nilai-circle-fill.kosong     { stroke: #E5E7EB; }
    ...
    /* ── Sidebar: kondisi belum ada nilai ────────────────────────── */
    .sidebar-belum-nilai {
        text-align: center;
        padding: 10px 0 4px;
    }
    ...
    /* ── Sidebar: Sedang Berlangsung badge ───────────────────────── */
    .badge-berlangsung {
        display: inline-flex;
        align-items: center;
        gap: 5px;
    ...
    /* ── Empty states di konten kanan ───────────────────────────── */

    /* State 1: Penilaian Sedang Berlangsung (ada yg sudah tapi belum semua) */
    .empty-berlangsung {
        background: var(--white);
        border: 1px solid var(--border);
        border-radius: var(--radius);
    ...
    /* State 2: Menunggu Penilaian per-tab (dosen tab ini belum input) */
    .empty-menunggu {
        background: var(--white);
        border: 1px solid var(--border);
        border-radius: var(--radius);
    ...
    /* Dot indikator di tab — dosen sudah input */
    ...
    /* Dot indikator — dosen sedang / belum */
    .tab-btn .tab-dot-pending {
        position: absolute;
        top: 4px; right: 6px;
        width: 6px; height: 6px;
        background: #F97316;
    ...
    .nilai-table tbody td:last-child {
        text-align: right;
        font-weight: 800;
        font-size: 14px;
    }
...
<div class="penilaian-wrap">
    ...
    {{-- ════════════════════════════════════════════════════════════════
         KONDISI 1: Belum ada satu pun dosen yang input nilai
         ════════════════════════════════════════════════════════════════ --}}
    ...
    <div class="empty-penilaian">
        ...
        <div class="empty-icon-wrap">
            ...
            <div class="empty-icon-dot">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="#fff" width="11" height="11">
                    <circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>
                </svg>
            </div>
        </div>
    ...
    {{-- ════════════════════════════════════════════════════════════════
         ...
         → Tampilkan grid sidebar + konten kanan
         ════════════════════════════════════════════════════════════════ --}}
    @else
    ...
    <div class="penilaian-grid">
        ...
        <div class="sidebar-kiri">
            <div class="nilai-card">
                ...
                @else
                    {{-- Sedang Berlangsung: sebagian dosen sudah input --}}
                    <div class="sidebar-belum-nilai">
                        ...
                        <div class="sidebar-belum-desc">Nilai akhir dan status kelulusan akan ditampilkan setelah seluruh dosen menyelesaikan penilaian seminar.</div>

                        <div class="sidebar-nilai-placeholder">
                            <div class="sidebar-placeholder-num">0</div>
                        ...
                        <div class="progress-dosen-wrap">
                            ...
                            <div class="progress-dosen-item">
                                ...
                                Pembimbing 1
                                @if($nilaiPembimbing1)
                                    <span class="progress-status-done">✓ Selesai</span>
                                @else
                                    <span class="progress-status-active">Menunggu</span>
                                @endif
                            </div>
                            <div class="progress-dosen-item">
                                ...
                                @if($nilaiPembimbing2)
                                    <span class="progress-status-done">✓ Selesai</span>
                                @else
                                    <span class="progress-status-pending">Menunggu</span>
                                @endif
                            </div>
                            <div class="progress-dosen-item">
        ...
        <div>
            ...
            <div class="tab-nav">
                @php
                    // Tentukan dot per tab
                    $dotPembimbing1 = $nilaiPembimbing1 !== null;
                    $dotPembimbing2 = $nilaiPembimbing2 !== null;
                    $dotPenguji1   = $nilaiPenguji1 !== null;
                @endphp
                ...
                <a href="?tab=pembimbing1" class="tab-btn {{ $tabAktif === 'pembimbing1' ? 'active' : '' }}">
                    Pembimbing 1
                    @if($dotPembimbing1)<span class="tab-dot-done"></span>
                    @elseif($sedangBerlangsung)<span class="tab-dot-pending"></span>
                    @endif
                </a>
                <a href="?tab=pembimbing2" class="tab-btn {{ $tabAktif === 'pembimbing2' ? 'active' : '' }}">
                    Pembimbing 2
                    @if($dotPembimbing2)<span class="tab-dot-done"></span>
                    @elseif($sedangBerlangsung)<span class="tab-dot-pending"></span>
                    @endif
                </a>
                <a href="?tab=penguji1"   class="tab-btn {{ $tabAktif === 'penguji1'    ? 'active' : '' }}">
                    Penguji 1
                    @if($dotPenguji1)<span class="tab-dot-done"></span>
                    @elseif($sedangBerlangsung)<span class="tab-dot-pending"></span>
                    @endif
                </a>
            </div>
            ...
            {{-- ════════════════════════════════════════════════════════
                 KONDISI 2: "Penilaian Sedang Berlangsung"
                 Tab Akumulasi ketika belum semua dosen input
                 ════════════════════════════════════════════════════════ --}}
            ...
            <div class="empty-berlangsung">
                ...
                <div class="empty-icon-wrap" style="background:#FFF7ED;border-color:#FED7AA;">
                    ...
                    <div class="empty-icon-dot" style="background:#F97316;">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="#fff" width="10" height="10">
                            <circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>
                        </svg>
                    </div>
                </div>
            ...
            {{-- ════════════════════════════════════════════════════════
                 KONDISI 3: "Menunggu Penilaian" per-tab
                 Dosen di tab ini belum input nilainya
                 ════════════════════════════════════════════════════════ --}}
            ...
            <div class="empty-menunggu">
                ...
                <div class="empty-desc">
                    Dosen belum menyelesaikan proses penilaian seminar tugas akhir.
                    Hasil penilaian akan tersedia setelah proses evaluasi seminar selesai dilakukan.
                </div>
            ...
            {{-- ════════════════════════════════════════════════════════
                 ...
                 ($adaPenilaian penuh, atau tab dosen yang sudah input)
                 ════════════════════════════════════════════════════════ --}}
            @else
            ...
            $modelsAll = array_filter([$nilaiPembimbing1, $nilaiPembimbing2, $nilaiPenguji1]);

            if ($tabAktif === 'akumulasi') {
                foreach ($komponenA as &$k) {
                    $k['nilai'] = count($modelsPembimbing) > 0
                        ? round(collect($modelsPembimbing)->avg(fn($m) => $m->{$k['key']} ?? 0))
                        : 0;
                }
                foreach ($komponenB as &$k) {
                    $k['nilai'] = count($modelsAll) > 0
                        ? round(collect($modelsAll)->avg(fn($m) => $m->{$k['key']} ?? 0))
                        : 0;
                }
                unset($k);
            ...
            $totalB = collect($komponenB)->filter(fn($k) => $k['nilai'] !== null)->sum('nilai');
            @endphp
            ...
            <div class="info-note-section">
                ...
                Tab Akumulasi menampilkan rata-rata nilai dari semua penilai. Section A dirata-rata dari pembimbing, Section B dari semua penilai.
            </div>
            @elseif($tabAktif === 'penguji1')
            <div class="info-note-section">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" width="14" height="14" style="flex-shrink:0;margin-top:1px;">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z"/>
                </svg>
                Penguji hanya menilai Section B (Penilaian Seminar TA-1). Section A tidak dinilai oleh penguji.
            </div>
            @endif
            ...
            <div class="penilaian-card">
                ...
                <table class="nilai-table">
                    <thead>
                        <tr>
                            <th>No</th><th>Kriteria Penilaian</th><th>Maks</th><th>Nilai</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($komponenA as $k)
