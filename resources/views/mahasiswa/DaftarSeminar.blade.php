...
<style>
    ...
    .seminar-wrap { background: var(--bg); min-height: 100vh; padding-bottom: 40px; }
    ...
    .hero-title { font-size: 28px; font-weight: 800; color: #735C00; margin-bottom: 6px; }
    .hero-sub { font-size: 13px; color: #92400E; margin-bottom: 16px; max-width: 500px; }
    ...
    .stat-value { font-size: 15px; font-weight: 800; color: var(--neutral); }
    .stat-sub   { font-size: 11px; color: var(--muted); margin-top: 3px; }
    .progress-bar-wrap { background: #F3F4F6; border-radius: 99px; height: 5px; margin-top: 6px; width: 100%; }
    .progress-bar-fill { background: var(--gold); border-radius: 99px; height: 100%; }

    .badge-sm { display: inline-block; font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 99px; }
    ...
    .badge-terdaftar{ background: #EFF6FF; color: #1D4ED8; border: 1px solid #BFDBFE; }
    .badge-na       { background: #F9FAFB; color: #9CA3AF; border: 1px solid #E5E7EB; }
    .badge-draft    { background: #FEF9C3; color: #6C5700; border: 1px solid #FDE68A; }
    ...
    .search-wrap svg { position: absolute; left: 28px; top: 50%; transform: translateY(-50%); color: var(--muted); }
    .search-input {
    ...
    .tabel-card { background: var(--white); border-radius: var(--radius); border: 1px solid var(--border); box-shadow: 0 2px 8px rgba(0,0,0,.04); overflow: hidden; }
    .tabel-title { padding: 16px 20px; border-bottom: 1px solid var(--border); font-size: 15px; font-weight: 800; color: var(--neutral); }
    ...
    .judul-ta { font-weight: 600; font-size: 13px; line-height: 1.4; max-width: 240px; }
    .tgl-label { font-size: 12px; font-weight: 700; color: var(--neutral); }
    .tgl-sub   { font-size: 11px; color: var(--muted); margin-top: 1px; }

    .progress-cell .persen { font-size: 11.5px; font-weight: 700; margin-bottom: 4px; }
    .bar-wrap  { background: #F3F4F6; border-radius: 99px; height: 5px; width: 110px; }
    .bar-fill  { border-radius: 99px; height: 100%; }
    .bar-green { background: #16A34A; }
    .bar-gold  { background: var(--gold); }
    .bar-red   { background: #DC2626; }

    .status-col { display: flex; flex-direction: column; gap: 5px; }

    .jadwal-info { display: flex; flex-direction: column; gap: 6px; }
    .jadwal-tgl  { font-size: 13px; font-weight: 700; color: var(--neutral); }
    .jadwal-row  { display: flex; align-items: center; gap: 6px; font-size: 12px; color: var(--muted); }
    ...
    .btn-perbaiki {
        ...
        border-radius: 10px; font-size: 12px; font-weight: 700;
        border: none; cursor: pointer; text-decoration: none;
        white-space: nowrap; text-align: center; display: inline-block; line-height: 1.4;
    }
    .btn-perbaiki:hover { background: #fdd835; color: #6C5700; }

    .empty-state { text-align: center; padding: 56px 20px; color: var(--muted); }
    .empty-state .icon { font-size: 40px; margin-bottom: 10px; }

    .popup-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,.5); z-index: 1060; align-items: center; justify-content: center; }
    .popup-overlay.show { display: flex; }
    .popup-box { background: #fff; border-radius: 20px; padding: 32px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 20px 60px rgba(0,0,0,.15); }
    .popup-icon  { font-size: 48px; margin-bottom: 12px; }
    .popup-title { font-size: 18px; font-weight: 800; color: var(--neutral); margin-bottom: 8px; }
    .popup-sub   { font-size: 13px; color: var(--muted); margin-bottom: 24px; line-height: 1.6; }
    .popup-actions { display: flex; gap: 10px; justify-content: center; }
    .btn-popup-cancel { padding: 10px 24px; border-radius: 99px; font-size: 13px; font-weight: 600; background: #F3F4F6; color: var(--muted); border: none; cursor: pointer; }
    .btn-popup-ok { padding: 10px 24px; border-radius: 99px; font-size: 13px; font-weight: 700; background: #FFE083; color: #6C5700; border: none; cursor: pointer; text-decoration: none; }
    .btn-popup-ok:hover { background: #fdd835; color: #6C5700; }
...
<div class="seminar-wrap">
    ...
    <div class="seminar-hero">
        <div class="hero-content">
            ...
            $adaPengajuanAktif = $pengajuans->contains(fn($p) =>
                $p->is_draft == 0 && $p->status_administrasi === 'Menunggu Verifikasi'
            );
            $adaDraft = $pengajuans->contains(fn($p) => $p->is_draft == 1);
            $adaSudahDaftar = $pengajuans->contains(fn($p) =>
                in_array($p->status_seminar, ['Menunggu Jadwal', 'Jadwal ditetapkan', 'Sudah Dijadwalkan', 'Selesai'])
            );
            $adaLolos = $pengajuans->contains(fn($p) =>
                $p->is_draft == 0 &&
                $p->status_administrasi === 'Lolos Administrasi' &&
                $p->status_seminar === 'Belum Daftar Seminar'
            );
            @endphp

            @if($adaSudahDaftar)
            <div style="display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,.85);color:#15803D;padding:10px 20px;border-radius:99px;font-size:13px;font-weight:700;border:1px solid #BBF7D0;">
    ...
    $sudahDaftar = $pengajuans->contains(
        fn($p) => in_array($p->status_seminar, ['Menunggu Jadwal', 'Jadwal ditetapkan', 'Sudah Dijadwalkan', 'Selesai'])
    );
    @endphp
        ...
        <div class="stat-card">
           ...
           <div>
                ...
                <div style="margin-top:6px;">
                    ...
                    <span class="badge-sm badge-na">Belum Mengajukan</span>
                    @elseif($statusAdministrasi === 'Lolos Administrasi')
                ...
                <div class="stat-sub" style="margin-top:4px;">
                    @if($pengajuans->isEmpty()) Belum ada pengajuan @else Terverifikasi Akademik @endif
                </div>
            </div>
        </div>
                        ...
                        <td>
                            ...
                            <div class="jadwal-info">
                                <div class="jadwal-tgl">
                                    {{ \Carbon\Carbon::parse($p->tanggal_seminar)->translatedFormat('d M Y') }}
                                </div>
                                @if($p->waktu_mulai)
                                <div class="jadwal-row">
                                    ...
                                    <span>{{ \Carbon\Carbon::parse($p->waktu_mulai)->format('H:i') }} — {{ $p->waktu_selesai ? \Carbon\Carbon::parse($p->waktu_selesai)->format('H:i') : '?' }} WIB</span>
                                </div>
                                @endif
                                @if($p->ruang)
                                <div class="jadwal-row">
