    ...
    <div class="filter-wrapper mb-3">
        ...
        <div class="filter-select-wrap">
            <select id="inputStatus">
                ...
                <option value="menunggu" {{ request('status_review') === 'menunggu' ? 'selected' : '' }}>
                    Menunggu Review
                </option>
                <option value="selesai" {{ request('status_review') === 'selesai' ? 'selected' : '' }}>
                    Selesai
                </option>
            </select>
        </div>
        ...
        <table class="tbl-reviewer" id="tblReviewer">
            ...
            <tbody id="tblBody">
                ...
                @php
                    ...
                    $catatanSingkat = $p->catatan
                        ? \Illuminate\Support\Str::limit($p->catatan, 30)
                        : '-';
                    ...
                    // Dosen Pembimbing
                    $dsn1 = $p->nidn_dsn1
                        ? $p->nidn_dsn1 . ($p->nama_dsn1 ? ' - ' . $p->nama_dsn1 : '')
                        : '-';
                    $dsn2 = $p->nidn_dsn2
                        ? $p->nidn_dsn2 . ($p->nama_dsn2 ? ' - ' . $p->nama_dsn2 : '')
                        : '-';
                @endphp
                <tr
                ...
                >
                    ...
                    <td>
                        @if($sudahReview)
                            <a href="{{ route('reviewer.proposal.detail', $p->id) }}"
        ...
        <form id="formReview" method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="mnew-field mb14">
                ...
                <textarea name="catatan"
                          id="reviewCatatan"
                          class="mnew-inp mnew-textarea-catatan"
                          rows="4"
                          maxlength="200"
            ...
            <div class="mnew-field mb14">
                ...
                <div class="mnew-upload-area" id="uploadArea" onclick="document.getElementById('reviewFile').click()">
                    ...
                    <div id="uploadPreview" style="display:none;">
                        ...
                        <button type="button" onclick="hapusFile(event)" style="
                            margin-left:10px; background:none; border:none;
                            color:#c0392b; cursor:pointer; font-size:0.85rem;
...
function applyFilter() {
    ...
    rows.forEach(function(row) {
        ...
        if (cocokSearch && cocokStatus && cocokTanggal) {
            row.style.display = '';
            visible++;
        } else {
